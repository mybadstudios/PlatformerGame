...
void FAnimNode_FootPlacementIK::CalculateEffector(float DeltaTime, USkeletalMeshComponent* SkelComp, FCSPose<FCompactPose>& MeshBases)
{
	...
	SkelComp->GetWorld()->LineTraceSingleByChannel(Hit, EndBoneWorldPos + TraceOffset, EndBoneWorldPos - TraceOffset, ECC_Pawn, FCollisionQueryParams(NAME_None, FCollisionQueryParams::GetUnknownStatId(), true, SkelComp->GetOwner()));

	// check if we should blend-in or blend-out this node
	const EMyBlendState OldBlendState = BlendState;
	if (Hit.GetHitObjectHandle().IsValid())
	{
