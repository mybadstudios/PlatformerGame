...
void UPlatformerGameUserSettings::ApplySettings(bool bCheckForCommandLineOverrides)
{
	...
	FAudioDevice* MainAudioDevice = (GEngine ? GEngine->GetMainAudioDevice().GetAudioDevice() : nullptr);
	if (MainAudioDevice)
	{
...
bool UPlatformerGameUserSettings::IsSoundVolumeDirty() const
{
	...
	if (MainAudioDevice)
	{
		...
		const float CurrentSoundVolume = MainAudioDevice->GetTransientPrimaryVolume();
